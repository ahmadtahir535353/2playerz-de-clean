...
class BingTopPagesWidget extends TableWidget
{
    ...
    public function table(Table $table): Table
    {
        ...
        return $table
            ->query($query)
            ->heading(__('messages.bing.top_performing_pages'))
            ->defaultSort('clicks', 'desc')
            ->columns([
                Tables\Columns\TextColumn::make('page')
                    ->label(__('messages.bing.page_url'))
                    ->searchable()
                    ->limit(50)
    ...
    {
        return __('messages.bing.top_performing_pages');
    }
