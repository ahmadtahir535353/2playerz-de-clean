...
class BingStatsOverviewWidget extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        if (!$token) {
            return [
                Stat::make(__('messages.bing.bing_webmaster'), __('messages.bing.not_connected'))
                    ->description(__('messages.bing.connect_in_settings'))
        ...
        $data = BingWebmasterData::where('token_id', $token->id)
        ...
        $prevData = BingWebmasterData::where('token_id', $token->id)
            ->where('data_type', 'overall')
        ...
        return [
            Stat::make(__('messages.bing.total_clicks'), number_format($totalClicks))
            ...
            Stat::make(__('messages.bing.total_impressions'), number_format($totalImpressions))
            ...
            Stat::make(__('messages.bing.average_ctr'), number_format($avgCtr, 2) . '%')
            ...
            Stat::make(__('messages.bing.average_position'), number_format($avgPosition, 1))
                ->description($positionChange <= 0 
                    ? number_format(abs($positionChange), 1) . '% ' . __('messages.bing.better')
                    : number_format($positionChange, 1) . '% ' . __('messages.bing.worse'))
