    ...
    static::saved(function ($post) {
        if ($post->scheduled_post == 1 && $post->scheduled_post_time) {
            ...
            if ($publishTime->isFuture()) {
                // Exact scheduled time pe dispatch karo, bina extra delay ke
                PublishScheduledPost::dispatch($post->id)->delay($publishTime);
            } else {
                // Agar time guzar gaya, toh abhi publish kar do
                $post->update([
                    'status' => 1,
                    'published_at' => now(),
                ]);
...
protected static function handleRecordUpdate(Model $record, array $data): Model
{
   ...
   Section::make('Videos')
    ->schema([
        Repeater::make('postMultipleVideos')
            ->relationship('postMultipleVideos')
            ->schema([
                Tabs::make('video_source')
                    ->tabs([
                        Tab::make('url_video')
                            ...
                            ->schema([
                                TextInput::make('video_url')
                                    ...
                                    ->afterStateUpdated(function ($state, \Filament\Forms\Set $set) {
                                        if (!empty($state)) {
                                            $videoId = Str::of($state)->after('watch?v=')->before('&');
                                            $query = parse_url($state, PHP_URL_QUERY);
                                            parse_str($query, $params);

                                            $startTime = 0;
                                            if (isset($params['t'])) {
                                                // 't' can be like '90s' or '1m30s' or '384'
                                                $timeStr = $params['t'];
                                                if (preg_match('/^(\d+)s$/', $timeStr, $matches)) {
                                                    $startTime = (int)$matches[1];
                                                } elseif (is_numeric($timeStr)) {
                                                    $startTime = (int)$timeStr;
                                                } elseif (preg_match('/(?:(\d+)m)?(?:(\d+)s)?/', $timeStr, $matches)) {
                                            ...
                                            if ($startTime > 0) {
                                                ...
                                                $embedUrl .= "?feature=oembed";
                                            }

                                            $set('video_embed_code', $embedUrl);
                                            $set('thumbnail_image_url', "https://i.ytimg.com/vi/{$videoId}/hqdefault.jpg");
                                        } else {
                                ...
                                TextInput::make('video_embed_code')
                                    ...
                                    ->live()
                                    ->required(function (Get $get) {
                                        return empty($get('uploadVideo')); // Required if no uploaded video
                                    })
                                    ->readOnly()
                                    ->url(),
                        ...
                        Tab::make('upload_video')
                            ...
                            ->schema([
                                SpatieMediaLibraryFileUpload::make('uploadVideo')
                                    ...
                                    ->maxSize(50000)
                                    ->required(function (Get $get) {
                                        return empty($get('video_url')); // Required if no URL
                                    }),
                            ]),
                    ])
                    ->persistTabInQueryString(),

                RichEditor::make('video_content')
            ...
            ->addActionLabel(__('messages.other_lang.add_another_video'))
            ->collapsible()
            ->defaultItems(1)
            ->maxItems(5), 
    ])
