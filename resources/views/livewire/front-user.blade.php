        ...
        @foreach ($posts as $post)
            <div class="col-sm-6 mb-5 d-flex">
                <div class="card w-100 ">
                    <a href="{{ route('detailPage', $post->slug) }}" class="w-100 h-100 d-flex flex-column">
                        <div class="position-relative">
                            <img src="{{ $post->post_image }}" class="card-img-top admin-card-image img-fluid"
                                alt="...">
                        ...
                        <div class="card-body admin-post-card-body">
                            ...
                            <p class="text-primary">
                                {{ $post->created_at->format('d') }}. {{ ucfirst(__('messages.common.' . strtolower($post->created_at->format('M')))) }}
                                 {{ $post->created_at->format('Y') }}</p>
