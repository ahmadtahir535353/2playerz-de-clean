...
@section('meta_image')
@php
// Priority order for video thumbnail
$metaImage = null;
if ($postDetail->postVideo) {
    // Priority 1: thumbnail_image_url (YouTube auto-fetch)
    if (!empty($postDetail->postVideo->thumbnail_image_url)) {
        $metaImage = $postDetail->postVideo->thumbnail_image_url;
    }
    // Priority 2: uploaded_thumb (manual upload)
    elseif (!empty($postDetail->postVideo->uploaded_thumb)) {
        $metaImage = $postDetail->postVideo->uploaded_thumb;
    }
    // Priority 3: Extract from video_content if YouTube embed
    elseif (!empty($postDetail->postVideo->video_content)) {
        if (preg_match('/youtube\.com\/embed\/([A-Za-z0-9_-]{11})/', $postDetail->postVideo->video_content, $matches)) {
            $metaImage = "https://i.ytimg.com/vi/{$matches[1]}/maxresdefault.jpg";
        }
    }
}
// Priority 4: Fallback to post_image
if (!$metaImage && !empty($postDetail->post_image)) {
    $metaImage = $postDetail->post_image;
}
// Priority 5: Default image
if (!$metaImage) {
    $metaImage = asset('front_web/images/default.jpg');
}
@endphp
{{ $metaImage }}
@endsection
...
@endsection
@push('jsonld')
@include('front_new.layouts.video-jsonld')
@endpush
@section('content')
@php
...
<style>
    /* Removed emojionearea styles - using native emoji picker now
    ...
    .dark-mode .emojionearea .emojionearea-editor {
                ...
                <div class="col-xl-8">
                    ...
                    <section class="news-details-left pe-xxl-3">
                        <div class="news-details">
                            ...
                            @foreach($postDetail->postMultipleVideos as $video)
                            <div class="news-content-img position-relative d-none">
                                <div class="d-flex justify-content-center">
                                    @if (!empty($video->uploaded_video))
                                    <video width="650" height="440" controls class="video-tag  col-12">
                                        <source src="{{ $video->uploaded_video }}">
                                    </video>
                                    @else
                                    <iframe src="{{ $video->video_embed_code }}" frameborder="1"
                                        width="auto" height="auto" id="embed_video_i_frame_{{ $loop->index }}"
                                        class="col-12"></iframe>
                                    @endif
                                </div>
                                <a href="#" class="tags position-absolute" style="background-color: {{ $postDetail->category->color ?? '#B051B0' }}; color: white;">{{ $postDetail->category->name }}</a>
                            </div>
                            ...
                            <div class="news-desc mb-20 video-content d-none">
                                <p>
                                    @php
                                        $html = $video->video_content ?? '';
                        ...
                        @foreach($postDetail->postMultipleVideos as $video)
                        @if (!empty($video->video_url) || (!empty($video->uploaded_video) && is_array($video->uploaded_video) && count($video->uploaded_video) > 0) && !empty(trim(strip_tags($video->video_content))))
                        <div class="news-content-img position-relative">
                            <div class="d-flex justify-content-center">
                                ...
                                @else
                                <iframe src="{{ $video->video_embed_code }}" frameborder="1"
                                    width="auto" height="440" id="embed_video_i_frame_{{ $loop->index }}"
                                    ...
                                    <section class="post-comment-section bg-light px-30 py-4 mb-5" style="box-shadow: 0px 0px 10px #00000020; border-radius: 15px;" id="commentFormSection">
                                        ...
                                        <form id="commentForm">
                                            ...
                                            <div class="row">
                                                @if (!Auth::check())
                                                <div class="col-md-12">
                                                    <a class="btn btn-primary" href="{{url('login')}}">Zum kommentieren
                                                        einloggen</a>
                                                </div>
                                                @else
                                                <div class="col-12">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="card d-flex flex-row mb-40">
                                            <div class="col-4 card-img-top ">
                                                <a href="{{ route('detailPage', $nextPost->slug) }}">
                                                    <img
                                                        src="@if( $nextPost->post_types == \App\Models\Post::VIDEO_TYPE_ACTIVE) {{$nextPost['postVideo']['thumbnail_image_url'] ?? ''}} @else {{$nextPost->post_image}} @endif"
                                                        height="100"
                                                        width="100">
                                                </a>
                                            </div>
                                        ...
                                        <div class="card position-relative slide-item">
                                            <div class="card-img-top">
                                                <a href="{{ route('detailPage', $relatedPost->slug) }}">
                                                    <img
                                                        src=" @if($relatedPost->post_types == \App\Models\Post::VIDEO_TYPE_ACTIVE) {{ $relatedPost['postVideo']['thumbnail_image_url'] ?? ''}} @else {{ $relatedPost['post_image'] }} @endif"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Style for <pre> tags
        const preElements = document.querySelectorAll('pre.language-markup');
        ...
        const phpElements = document.querySelectorAll('pre.language-php');
            phpElements.forEach(element => {
                ...
                if (urlMatch) {
                    ...
                    .then(data => {
                        ...
                        const { title, image, description, video_embed } = data;
...
<script>
    // Image lightbox with prev/next (Insider Gaming style)
    document.addEventListener('DOMContentLoaded', function() {
        const imageElements = [];
        document.querySelectorAll('.video-content').forEach(section => {
            section.querySelectorAll('img').forEach(im => imageElements.push(im));
        });
        ...
        imageElements.forEach((img, index) => {
            ...
            if (copyrightText && copyrightText.trim() !== '' && !img.classList.contains('detail-gallery-img')) {
                if (!img.nextElementSibling || !img.nextElementSibling.classList.contains('image-copyright')) {
                    ...
                    copyrightDiv.style.cssText = 'position:absolute;bottom:5px;right:10px;color:#fff;font-size:12px;font-weight:500;z-index:10;pointer-events:none;text-shadow:1px 1px 2px rgba(0,0,0,0.8);';
