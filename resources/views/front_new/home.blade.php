...
@section('pageCss')
    ...
    <style>
        /* LCP Hero Image Optimization - Prevent Layout Shift */
        .hero-image {
            aspect-ratio: 16 / 9;
            min-height: 425px;
...
@endsection
@push('preload')
@if(isset($sliderPosts) && $sliderPosts->isNotEmpty())
    @php
        $firstPost = $sliderPosts->first();
        $firstImage = null;
        if ($firstPost->post_types == \App\Models\Post::VIDEO_TYPE_ACTIVE) {
            $firstImage = !empty($firstPost->postVideo) && !empty($firstPost->postVideo->thumbnail_image_url) 
                ? $firstPost->postVideo->thumbnail_image_url 
                : (!empty($firstPost->postVideo) && !empty($firstPost->postVideo->uploaded_thumb) 
                    ? $firstPost->postVideo->uploaded_thumb 
                    : asset('front_web/images/default.jpg'));
        } else {
            $firstImage = $firstPost->post_image;
        }
    @endphp
    @if($firstImage)
...
@if(session('message'))
    <div id="custom-toast" class="custom-toast">
        ...
        <div class="close-btn" onclick="document.getElementById('custom-toast').remove()">×</div>
...
@section('content')
<h1 class="visually-hidden">Gaming News, Reviews & Updates</h1>
    <div class="home-page">
        <!-- start hero section -->
                                ...
                                @foreach ($sliderPosts as $sliderPost)
                                    <div
                                        ...
                                        <a href="{{ route('detailPage', $sliderPost->slug) }}">
                                            {{--                                            <img data-src="{{ $sliderPost->post_image }}" src="{{ asset('front_web/images/bg-process.png') }}" class="w-100 h-100 lazy" alt="{{ $sliderPost->title }}"/> --}}
                                            @if ($sliderPost->post_types == \App\Models\Post::AUDIO_TYPE_ACTIVE)
                                                ...
                                                <img src="{{ $sliderPost->post_image }}" 
                                                     class="w-100 h-100"
                                                     alt="{{$sliderPost->title}}" 
                                                     style="filter: brightness(0.6);"
                                                     @if($loop->iteration <= 1) fetchpriority="high" loading="eager" @else loading="lazy" @endif
                                                     decoding="async"
                                                     width="1200"
                                                     height="675">
                                        ...
                                        <a href="{{ route('categoryPage', $sliderPost->category->slug) }}" 
                                            class="tags position-absolute fw-7 {{ getColorClass($sliderPost->category->id) }}" style="background-color: {{ $sliderPost->category->color ?? '#B051B0' }}; color: white;">{!! $sliderPost->category->name !!}</a>
                                        <div class="hero-content position-absolute px-40 mb-sm-4 mb-3 w-100">
                                            <h3 class="text-white pb-2"><a
                                                    href="{{ route('detailPage', $sliderPost->slug) }}"
                                                    class="text-decoration-none text-white">{!! \Illuminate\Support\Str::limit($sliderPost->title, 85, '...') !!}</a></h3>
                                            <div class="desc d-sm-flex align-items-center justify-content-between">
                                                <p class="fs-14 text-white mb-sm-0 mb-1"><a
                                                        href="{{ route('user.public.profile', optional($sliderPost->user)->username ?? $sliderPost->user->id) }}"
                                                        class="text-white">{{ __('messages.common.by') }}
                                                        {{ $sliderPost->user->full_name }}</a></p>
                                                <div class="desc d-flex">
                                                    <p class="fs-14 text-white mb-0">
                                                        {{ $sliderPost->created_at->format('d') }}. {{ ucfirst(__('messages.common.' . strtolower($sliderPost->created_at->format('F')))) }}
                                                        {{ $sliderPost->created_at->format('Y') }}</p>
                                                    <span class=" text-primary px-sm-4 px-2"> | </span>
                                                    <a href="{{ route('detailPage', $sliderPost->slug) }}#blog-post-comment-view-section" class="text-white text-decoration-none">
                                                        <p class="fs-14 text-white mb-0"><i class="fa-solid fa-comments fs-12 text-gray me-1"></i> {{ $sliderPost->comment_count }}</p>
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="card position-relative">
                                        <div class="card-img-top">
                                            <a href="{{ route('detailPage', $row->slug) }}">
                                                {{--                                                <img data-src="{{ $row->post_image }}" src="{{ asset('front_web/images/bg-process.png') }}" alt="{{ $row->title }}" class="w-100 h-100 lazy"> --}}
                                                @if ($row->post_types == \App\Models\Post::AUDIO_TYPE_ACTIVE)
                                                    <button class="common-music-icon small-music-icon" type="button">
                                                        <i class="icon fa-solid fa-music text-white"></i>
                                                    </button>
                                                    <img src="{{ $row->post_image }}" class="w-100 h-100" alt="{{ $row->title }}" />
                                                @elseif($row->post_types == \App\Models\Post::VIDEO_TYPE_ACTIVE)
                                                    ...
                                                    <img src="{{ !empty($thumbUrl) ? $thumbUrl : $thumbImage }}"
                                                        class="w-100 h-100" alt="thumbnil" />
                                                @else
                                                    <img src="{{ $row->post_image }}" class="w-100 h-100" alt="{{ $row->title }}" />
                                                @endif
                                            </a>
                                        ...
                                        <a href="{{ route('categoryPage', $row->category->slug) }}"
                                            class="tags position-absolute  fw-7 {{ getColorClass($row->category->id) }}" style="background-color: {{ $row->category->color ?? '#B051B0' }}; color: white;">{!! $row->category->name !!}</a>
                                        <div class="card-body">
                                            ...
                                            <div class="d-flex justify-content-between">
                                                <span
                                                class="card-text fs-12 text-gray">{{ $row->created_at->format('d') }}. {{ ucfirst(__('messages.common.' . strtolower($row->created_at->format('M')))) }}
                                                 {{ $row->created_at->format('Y') }}</span>
                                                <a href="{{ route('detailPage', $row->slug) }}#blog-post-comment-view-section">
                                                    <p class="fs-14 text-gray mb-0"><i class="fa-solid fa-comments fs-12 text-gray me-1"></i> {{ $row->comment_count }}</p>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="row">
                            ...
                            @else
                                <div class="container index-top-desktop">
                                    <a href="{{ $adImageDesktopTop->ad_url }}" target="_blank">
                                        <img src="{{ asset($adImageDesktopTop->ad_banner) }}" width="800"
                                            class="img-fluid">
                                    </a>
                                </div>
                            @endif
                            ...
                            @else
                                <div class=" container index-top-mobile">
                                    <a href="{{ $adImageMobileTop->ad_url }}" target="_blank">
                                        <img src="{{ asset($adImageMobileTop->ad_banner) }}" width="350"
                                            class="img-fluid">
                                    </a>
                                </div>
                            @endif
                            ...
                            @if (!empty($firstHeadlinePost))
                                <div class="section-heading pb-3">
                                    <div class="row align-items-center justify-content-between">
                                        <div class="col-md-5 section-heading-left section-heading">
                                                        ...
                                                        @foreach ($catePost->where('visibility', \App\Models\Post::VISIBILITY_ACTIVE)->skip(1)->take(2) as $posts)
                                                            <div class="card mb-4 pb-sm-2 me-lg-0 pe-sm-1 me-sm-2">
                                                                ...
                                                                <div class="card-body">
                                                                 ...
                                                                 <div class="d-flex justify-content-between">   
                                                                    <span
                                                                        class="card-text fs-12 text-gray">{{ $posts->created_at->format('d') }}. {{ ucfirst(__('messages.common.' . strtolower($posts->created_at->format('M')))) }}
                                                                         {{ $posts->created_at->format('Y') }}</span>
                                                                        <a href="{{ route('detailPage', $posts->slug) }}#blog-post-comment-view-section" >
                                                                                <p class="fs-14 text-gray mb-0"><i class="fa-solid fa-comments text-gray"></i> {{ $posts->comment_count }}</p>
                                                                        </a>
                                                                 </div>       
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div
                                                                ...
                                                                <div
                                                                    ...
                                                                    <p class="fs-14 text-white mb-sm-0 mb-1">
                                                                        {{ $middlePost->created_at->format('d') }}. {{ ucfirst(__('messages.common.' . strtolower($middlePost->created_at->format('M')))) }}
                                                                         {{ $middlePost->created_at->format('Y') }}</p>
                                                                    <a href="{{ route('detailPage', $middlePost->slug) }}#blog-post-comment-view-section">
                                                                                <p class="fs-14 text-white mb-0"><i class="fa-solid fa-comments text-gray"></i> {{ $middlePost->comment_count }}</p>
                                                                    </a>
                                ...
                                <section class="technology-section">
                                    <div class="section-heading border-bottom-0">
                                        <div class="row align-items-center">
                                            <div class="col-md-6 section-heading-left section-heading">
                                                @php
                                                    // Check if category is "News" - link to /p
                                                    $isNewsCategory = strtolower($category->name) === 'news' || strtolower($category->slug ?? '') === 'news';
                                                @endphp
                                                @if($isNewsCategory)
                                                    <h2 class="text-black mb-0">
                                                        <a href="{{ route('allPosts') }}" class="text-black text-decoration-none">{!! $category->name !!}</a>
                                                    </h2>
                                                @else
                                                    <h2 class="text-black mb-0">
                                                        <a href="{{ route('categoryPage', $category->slug ?? '') }}" class="text-black text-decoration-none">{!! $category->name !!}</a>
                                                    </h2>
                                                @endif
                                            </div>
                                        </div>
                                                ...
                                                <div
                                                    ...
                                                    <div class="post-image position-relative rounded-10">
                                                        <a href="{{ route('detailPage', $categoryPost->slug) }}">
                                                            ...
                                                            {{--                                                                 alt="{{ $categoryPost->title }}" src="{{ asset('front_web/images/bg-process.png') }}" --}}
                                                            {{--                                                                 class="w-100 h-100 lazy"/> --}}
                                                            @if ($categoryPost->post_types == \App\Models\Post::AUDIO_TYPE_ACTIVE)
                                                                ...
                                                                <img src="{{ $categoryPost->post_image }}"
                                                                    class="w-100 h-100" alt="{{ $categoryPost->title }}" />
                                                        ...
                                                        <div class="post-content position-absolute px-30 mb-3 pb-1 w-100">
                                                            ...
                                                            <div
                                                                ...
                                                                <p class="fs-12 text-white mb-sm-0 mb-1">
                                                                    <a href="{{ route('user.public.profile', optional($categoryPost->user)->username ?? $categoryPost->user->id) }}"
                                                                        class="text-white">{{ __('messages.common.by') }}
                                                                        {{ $categoryPost->user->full_name }}</a>
                                                                </p>
                                                                <p class="fs-12 text-white mb-0">
                                                                    {{ $categoryPost['created_at']->format('d') }}. {{ ucfirst(__('messages.common.' . strtolower($categoryPost['created_at']->format('M')))) }}
                                                                     {{ $categoryPost['created_at']->format('Y') }}</p>
                                                                    <a href="{{ route('detailPage', $categoryPost->slug) }}#blog-post-comment-view-section">
                                                                                <p class="fs-14 text-white mb-0"><i class="fa-solid fa-comments text-gray"></i> {{ $categoryPost->comment_count }}</p>
                                                                    </a>
                                                            </div>
                                                        </div>
                                                    </div>
                        ...
                        @else
                            <div class="container index-top-desktop">
                                ...
                                <a href="{{ $adImageDesktopBottom->ad_url }}" target="_blank">
                                    <img src="{{ asset($adImageDesktopBottom->ad_banner) }}" width="800"
                                        class="img-fluid">
                                </a>
                            </div>
                        @endif
                        ...
                        @else
                            <div class=" container index-top-mobile">
                                <a href="{{ $adImageMobileBottom->ad_url }}" target="_blank">
                                    <img src="{{ asset($adImageMobileBottom->ad_banner) }}" width="350"
                                        class="img-fluid">
                                </a>
                            </div>
                        @endif
                    ...
                    @if (isset($latestPosts) && !$latestPosts->isEmpty())
                        <section class="latest-news-section pt-60">
                            <div class="section-heading border-bottom-0">
                                <div class="row align-items-center">
                                    <div class="col-sm-6 section-heading-left section-heading">
                                        <h2 class="text-black mb-0">{{ __('messages.details.latest_news') }}</h2>
                                    </div>
                                    <div class=" col-sm-6 text-end">
                                        <a href="{{ route('allPosts') }}"
                                            class="fs-14 btn fw-6 btn-primary text-white" style="border: none;">{{ __('messages.details.view_more') }}</a>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <div class="card position-relative">
                                                <div class="news-post-image rounded-10">
                                                    <a href="{{ route('detailPage', $latestPost->slug) }}">
                                                        {{--                                                            <img data-src="{{$latestPost->post_image}}" alt="{{$latestPost->title}}" src="{{ asset('front_web/images/bg-process.png') }}" class="w-100 h-100 lazy"> --}}
                                                        @if ($latestPost->post_types == \App\Models\Post::AUDIO_TYPE_ACTIVE)
                                                            ...
                                                            <img src="{{ $latestPost->post_image }}"
                                                                class="w-100 h-100" alt="{{ $latestPost->title }}" />
                                                ...
                                                <a href="{{ route('categoryPage', $latestPost->category->slug) }}"
                                                    class="tags position-absolute fw-7"
                                                    style="background-color: {{ $latestPost->category->color ?? '#B051B0' }}; color: white;">
                                                ...
                                                <div class="news-post-content">
                                                    ...
                                                    <div class="desc d-flex">
                                                        ...
                                                        <p class="fs-14 text-black mb-0"><a
                                                                href="{{ route('user.public.profile', optional($latestPost->user)->username ?? $latestPost->user->id) }}"
                                                                class="text-black">{{ __('messages.common.by') }}
                                                                {{ $latestPost->user->full_name }}</a>
                                                        </p>
                                                        <span class=" text-primary  px-2"> | </span>
                                                        <p class="fs-14 text-black mb-0">
                                                            {{ $latestPost->created_at->format('d') }}. {{ ucfirst(__('messages.common.' . strtolower($latestPost->created_at->format('M')))) }}
                                                             {{ $latestPost->created_at->format('Y') }}</p>

                                                            <span class=" text-primary  px-2"> | </span>

                                                            <a href="{{ route('detailPage', $latestPost->slug) }}#blog-post-comment-view-section" >
                                                                                <p class="fs-14 text-gray mb-0"><i class="fa-solid fa-comments text-gray"></i> {{ $latestPost->comment_count }}</p>
                                                            </a>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div class="col-lg-3 d-lg-block d-sm-flex">
                                    @foreach ($featCatePost->where('featured', 1)->where('visibility', \App\Models\Post::VISIBILITY_ACTIVE)->skip(1)->take(2) as $posts)
                                        ...
                                        <div class="card mb-4 pb-sm-2 me-lg-0 pe-sm-1 me-sm-2">
                                            <div class="card-img-top ">
                                                <a href="{{ route('detailPage', $posts->slug) }}">
                                                    {{--                                                            <img data-src="{{$posts->post_image}}" alt="{{$posts->title}}" src="{{ asset('front_web/images/bg-process.png') }}" class="w-100 h-100 lazy"> --}}
                                                    @if ($posts->post_types == \App\Models\Post::AUDIO_TYPE_ACTIVE)
                                            ...
                                            <div class="card-body">
                                                ...
                                                <span
                                                    class="card-text fs-12 text-gray">{{ $posts->created_at->format('d') }}. {{ ucfirst(__('messages.common.' . strtolower($posts->created_at->format('M')))) }}
                                                     {{ $posts->created_at->format('d') }}</span>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                                <div class="col-lg-6 mb-lg-0">
                                    ...
                                    <div class="featured-post-image position-relative rounded-10">
                                        <a href="{{ route('detailPage', $middleFeaturePost->slug) }}">
                                            ...
                                            @if ($middleFeaturePost->post_types == \App\Models\Post::AUDIO_TYPE_ACTIVE)
                                                ...
                                                <img src="{{ $middleFeaturePost->post_image ? $middleFeaturePost->post_image : ' /' }}"
                                                    class="w-100 h-100" alt="{{ $middleFeaturePost->title }}" />
                                        ...
                                        <div class="featured-post-content position-absolute px-30 mb-3 pb-1">
                                            ...
                                            <div class="desc d-sm-flex align-items-center justify-content-between">
                                                <p class="fs-14 text-white mb-sm-0 mb-1">
                                                    {{ $middleFeaturePost->created_at->format('d') }}. {{ ucfirst(__('messages.common.' . strtolower($middleFeaturePost->created_at->format('M')))) }}
                                                     {{ $middleFeaturePost->created_at->format('Y') }}</p>
                                                    <span class=" text-primary  px-2"> | </span>
                                                    <a href="{{ route('detailPage', $middleFeaturePost->slug) }}#blog-post-comment-view-section">
                                                                                <p class="fs-14 text-white mb-0"><i class="fa-solid fa-comments text-gray"></i> {{ $middleFeaturePost->comment_count }}</p>
                                                    </a>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="blog">
                    ...
                    <div class="email-box position-relative">
                        ...
                        <form action="{{ route('subscribe.store') }}" method="POST"
                            class="subscribe-form-style-2" id="subscriberForm">
                            @csrf
                            <input type="email" class="fs-14 text-gray subscribe-form" name="email"
                                id="email_2" placeholder="{{ __('messages.common.your_email') }}" required>
                            <button type="submit"
                                class="button btn-primary d-none d-sm-block position-absolute fs-14 subscribe-btn">
