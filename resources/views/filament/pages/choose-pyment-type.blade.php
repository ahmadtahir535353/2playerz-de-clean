<section class="flex flex-col py-8 gap-y-8">
    ...
    <div class="py-6 border border-gray-200 rounded-lg dark:border-white/10">
        <div class="flex flex-col justify-center gap-4 px-4 md:flex-row">
            @php
                $currentActivePlan = getCurrentPlanDetails();
                $currentPlan = $currentActivePlan['currentPlan'];
            @endphp
            @if ($currentActivePlan !== null && !$currentActivePlan['trial_ends_at'])
                <div
                    ...
                    <div class="">
                        <div class="flex items-center py-2">
                            ...
                            <span class="w-1/2 text-gray-600 dark:text-gray-400">{{ $currentActivePlan['name'] }}</span>
                        </div>
                        <div class="flex items-center py-2">
                            <h4 class="w-1/2 font-bold text-gray-600 dark:text-gray-200">
                                {{ __('messages.subscription.plan_price') }}</h4>
                            <span
                                class="w-1/2 text-gray-600 dark:text-gray-400">{{ $currentPlan->currency->currency_icon }}
                                {{ number_format($currentPlan->price) }}</span>
                        </div>
                        <div class="flex items-center py-2">
                            <h4 class="w-1/2 font-bold text-gray-600 dark:text-gray-200">
                                {{ __('messages.subscription.start_date') }}</h4>
                            <span
                                class="w-1/2 text-gray-600 dark:text-gray-400">{{ date('d/m/Y', strtotime($currentActivePlan['startAt'])) }}</span>
                        </div>
                        <div class="flex items-center py-2">
                            <h4 class="w-1/2 font-bold text-gray-600 dark:text-gray-200">
                                {{ __('messages.subscription.end_date') }}</h4>
                            <span
                                class="w-1/2 text-gray-600 dark:text-gray-400">{{ date('d/m/Y', strtotime($currentActivePlan['endsAt'])) }}</span>
                        </div>
                        <div class="flex items-center py-2">
                            <h4 class="w-1/2 font-bold text-gray-600 dark:text-gray-200">
                                {{ __('messages.subscription.used_days') }}</h4>
                            <span
                                class="w-1/2 text-gray-600 dark:text-gray-400">{{ number_format($currentActivePlan['usedDays']) }}
                        ...
                        <div class="flex items-center py-2">
                            ...
                            <span
                                class="w-1/2 text-gray-600 dark:text-gray-400">{{ number_format($currentActivePlan['remainingDays']) }}
                                {{ __('messages.subscription.days') }}</span>
                        </div>
                        <div class="flex items-center py-2">
                            <h4 class="w-1/2 font-bold text-gray-600 dark:text-gray-200">
                            ...
                            <span
                                class="w-1/2 text-gray-600 dark:text-gray-400">{{ $currentPlan->currency->currency_icon }}{{ $currentActivePlan['usedBalance'] }}</span>
                        </div>
                        <div class="flex items-center py-2">
                            <h4 class="w-1/2 font-bold text-gray-600 dark:text-gray-200">
                                {{ __('messages.subscription.remaining_balance') }}</h4>
                            <span
                                class="w-1/2 text-gray-600 dark:text-gray-400">{{ $currentPlan->currency->currency_icon }}{{ $currentActivePlan['remainingBalance'] }}</span>
                        </div>
                    </div>
                </div>
            @endif
            <div
                ...
                <div class="pb-4">
                    <h3 class="">
                        <span
                            class="text-2xl font-bold text-primary-400 dark:text-primary-600">{{ __('messages.plans.new_plan') }}</span>
                    </h3>
                </div>
                <div class="">
                    <div class="flex items-center py-2">
                    ...
                    <div class="flex items-center py-2">
                        <h4 class="w-1/2 font-bold text-gray-600 dark:text-gray-200">
                            {{ __('messages.subscription.plan_price') }}</h4>
                        <span
                            class="w-1/2 text-gray-600 dark:text-gray-400">{{ $plan->currency->currency_icon }}{{ $plan->price }}</span>
                    </div>
                    <div class="flex items-center py-2">
                        <h4 class="w-1/2 font-bold text-gray-600 dark:text-gray-200">
                            {{ __('messages.subscription.start_date') }}</h4>
                    ...
                    @if ($currentActivePlan !== null)
                        <div class="flex items-center py-2">
                            ...
                            <span
                                class="w-1/2 text-gray-600 dark:text-gray-400">{{ $plan->currency->currency_icon }}{{ $newPlan['remainingBalance'] }}</span>
                        </div>
                    @endif
                    <div class="flex items-center py-2">
                        <h4 class="w-1/2 font-bold text-gray-600 dark:text-gray-200">
                        ...
                        <span
                            class="w-1/2 text-gray-600 dark:text-gray-400">{{ $plan->currency->currency_icon }}{{ $newPlan['amountToPay'] }}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="flex justify-center">
            <div class="w-full max-w-sm pt-6">
                {{-- Payable Amount 0 --}}
                {{-- @dd($paymentAmount,$newPlan['amountToPay']<= 0) --}}
                ...
                <div class="">
                    ...
                    @elseif ($paymentType == 1)
                        <form action="{{ route('purchase-subscription') }}" method="POST" class="flex justify-center">
                            @csrf
                    ...
                    @elseif ($paymentType == 2)
                        <form action="{{ route('paypal.init') }}" method="POST" class="flex justify-center">
                            @csrf
                            <input type="hidden" name="plan" value="{{ $plan }}">
                            <div class="pt-4">
                                <x-filament::button wire:loading.attr="disabled" type="submit" class="px-4">
