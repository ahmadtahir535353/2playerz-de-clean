...
@section('content')
    ...
    <div class="">
        ...
        <div
            class="audio-player fixed bottom-0 w-full flex items-center justify-between p-4 border bg-gray-100 md:gap-6 gap-4 z-10 hidden">
            <div class="sm:flex hidden items-center gap-3">
                <img alt="player_img" class="relative rounded-full w-16 h-16 min-w-[64px] object-cover player-img"
                    id="playImage" src="{{ $postDetail->post_image }}" />
                <div class="flex-grow overflow-hidden">
                    <h3 class="text-sm sm:text-base font-medium truncate">
                        {{ $postDetail->title }}
                    </h3>
                    <span class="block text-xs text-neutral-500 dark:text-neutral-400">{{ $postDetail->slug }}</span>
                </div>
            </div>
            <div class="flex items-center w-full md:gap-6 gap-4 justify-end" x-data="{}">
                @foreach ($postDetail->postAudios->media as $audio)
                    <audio x-ref="audio" controls id="audio" class="w-full">
                        <source src="{{ $audio->original_url }}" type="audio/mpeg" />
                    </audio>
                @endforeach
                <button
                    class="close-btn flex-shrink-0 flex items-center justify-center rounded-full focus:outline-none focus:shadow-outline border scale-90 hover:scale-100 w-10 h-10">
        ...
        <div class="container mx-auto max-w-7xl xl:px-0 lg:px-10 md:px-8 px-4">
            <div class="">
                ...
                <div class="sm:pt-5 flex flex-wrap">
                    <div
                        class="flex items-center {{ getFrontSelectLanguageIsoCode() == 'ar' ? 'ml-10' : 'mr-10' }} sm:pt-0 pt-3">
                        <div
                            class="w-8 h-8 rounded-full overflow-hidden {{ getFrontSelectLanguageIsoCode() == 'ar' ? 'ml-4' : 'mr-4' }}">
                    ...
                    <div class="flex flex-wrap sm:pt-0 pt-3">
                        <div class="flex items-center {{ getFrontSelectLanguageIsoCode() == 'ar' ? 'ml-10' : 'mr-10' }}">
                            <div class="w-4 h-4 {{ getFrontSelectLanguageIsoCode() == 'ar' ? 'ml-2.5' : 'mr-2.5' }}">
                        ...
                        <div class="flex items-center {{ getFrontSelectLanguageIsoCode() == 'ar' ? 'ml-10' : 'mr-10' }}">
                            ...
                            <span
                                class="text-gray-200 sm:text-base text-sm font-medium">{{ getReadingTime($postDetail->postArticle?->article_content ? $postDetail->postArticle->article_content : $postDetail->description) }}</span>
                        </div>
            ...
            <div class="lg:grid lg:grid-cols-3 pt-10 gap-8">
                <div class="col-span-2 sm:mb-0">
                    ...
                    <div class="w-full sm:h-[548px] h-[405px] rounded-xl overflow-hidden relative mb-10">
                        <img src="{{ $postDetail->post_image }}" class="w-full h-full object-cover" loading="lazy" />
                        ...
                        <div class="absolute top-1/2 left-1/2">
                            <button type="button" class="relative rounded-xl group cursor-pointer play-btn">
                                <div class="absolute inset-0 flex items-center justify-center p-8">
                                    <div class="w-full h-full transition duration-300 ease-in-out bg-cyan-500 filter">
                                    </div>
                                </div>
                                <div
                                    class="absolute inset-0 flex items-center justify-center transition duration-200 ease-in-out rounded-xl">
                                    <div class="rounded-full p-2 border playing-button">
                                        <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none"
                                            ...
                                            <path
                                                ...
                                                stroke="#ffffff" stroke-width="1.5" stroke-linecap="round"
                                                stroke-linejoin="round"></path>
                                            <path d="M20.8398 16.8V12" stroke="#ffffff" stroke-width="1.5"
                                            ...
                                            <path
                                                ...
                                                stroke="#ffffff" stroke-width="1.5" stroke-linecap="round"
                                                stroke-linejoin="round"></path>
                                            <path d="M9.40039 9.5199L20.8404 6.3999" stroke="#ffffff" stroke-width="1.5"
                                                stroke-linecap="round" stroke-linejoin="round"></path>
                                        </svg>
                                    </div>
                                </div>
                    ...
                    <div class="sm:flex justify-between items-center relative">
                        ...
                        <div class="flex gap-6 px-2 sm:pt-0 pt-4">
                                            ...
                                            @foreach ($emojis as $emoji)
                                                <a href="javascript:void(0);" class="flex flex-col items-center px-1.5">
                                                    <div class="text-4xl relative emoji-id"
                                                        ...
                                                        <span
                                                            class="bg-black {{ getFrontSelectLanguageIsoCode() == 'ar' ? 'p-0' : '' }} rounded-full text-white text-[10px] absolute -top-1 -right-1 w-4 h-4 flex justify-center items-center post-reaction-count  like-reaction"
                            ...
                            @if (getSettingValue()['facebook'] ||
                                    getSettingValue()['twitter'] ||
                                    getSettingValue()['linkedin'] ||
                                    getSettingValue()['reddit'] ||
                            ...
                            <div class="grid lg:grid-cols-3 gap-8">
                                <div class="comment-container lg:col-span-3 light-gray-bg rounded-xl sm:p-8 p-5">
                                    ...
                                    <form class="" id="commentFormTailwind">
                                        ...
                                        <div class="">
                                            <button
                                                class="bg-primary text-white rounded-lg font-bold text-sm py-3 px-10 commentBtn">
                            ...
                            @foreach ($comments as $comment)
                                <div class="xs:flex flex sm:flex card-view-{{ $comment->id }} border-b py-5">
                                    ...
                                    @if (Auth::check() && $comment->user_id == getLogInUser()->id)
                                        <div
                                            class="{{ getFrontSelectLanguageIsoCode() == 'ar' ? 'mr-auto pl-5' : 'ml-auto pr-5' }}">
                                            <button
                ...
                <div class="grid md:grid-cols-2 gap-8">
                    <div class="md:max-w-[486px]">
                        @if (!empty($previousPost))
                            <div class="flex">
                                <div class="sm:mr-5 {{ getFrontSelectLanguageIsoCode() == 'ar' ? 'ml-4' : 'mr-4' }}">
                    ...
                    <div class="md:max-w-[486px] ms-auto">
                        @if (!empty($nextPost))
                            <div class="flex">
                                <div
                                    class="sm:w-[150px] w-[115px] sm:h-[100px] h-[90px] sm:min-w-[150px] min-w-[115px] rounded-lg overflow-hidden {{ getFrontSelectLanguageIsoCode() == 'ar' ? 'sm:ml-5 ml-4' : 'sm:mr-5 mr-4' }}">
