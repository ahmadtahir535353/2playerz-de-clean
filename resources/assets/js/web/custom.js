document.addEventListener("DOMContentLoaded", loadWebCustom);
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success) {
                ...
                $(".comment-count-display").text(result.data.commentCount);
                ...
                console.log(result.data.commentView.parent_id);
                 if(result.data.commentView.parent_id == null){
                setTimeout(() => {
                    // let $target = $('#comment-' + result.data.commentView.id);
                    // let $target = $('#comment-212');
                   
                    $('html, body').animate({
                        scrollTop: $('.comment-view').offset().top -10
                    }, 500);
                ...
                let data = {
                    ...
                    commentCreated: result.data.commentCreated,
                    getFrontSelectLanguageIsoCode: lang,
                };

                // Clear comment textarea (removed emojionearea reference)
                $('#comment').val('');
